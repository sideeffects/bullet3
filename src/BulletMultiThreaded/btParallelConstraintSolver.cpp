			...
			for(uint32_t batchId=0;batchId<contactParallelGroup->numBatches[phaseId];batchId++) {
				...
				if(batchId%numTasks == taskId && numPairs > 0) {
					...
					for(uint32_t i=0;i<numPairs;i++) {
						...
						btPersistentManifold& contact = *dispatcher->getManifoldByIndexInternal(contactIndex);
	...
	float timeStep)
{
	for(uint32_t i=0;i<numContactPairs;i++) {
		...
		btPersistentManifold& contact = *dispatcher->getManifoldByIndexInternal(id);
...
void	SolverThreadFunc(void* userPtr,void* lsMemory)
{
	...
	switch(io->cmd) {
		...
		CustomSolveConstraintsTaskParallel(
			...
			io->solveConstraints.dispatcher,
			...
			while(!empty) {
				...
				if(batch > 0) {
					CustomSetupContactConstraintsTask(
						...
						io->setupContactConstraints.dispatcher,
	...
	)
{
	...
	for(int t=0;t<maxTasks;t++) {
		...
		io[t].setupContactConstraints.dispatcher = dispatcher;	
...
//#define SEQUENTIAL_SETUP
		...
		CustomSetupContactConstraintsTask(contactPairs1,numContactPairs,dispatcher,offsetRigStates,offsetSolverBodies,numRigidBodies,separationBias,timeStep);
#else
...
//#define SOLVE_SEQUENTIAL
		...
		for(int t=0;t<maxTasks;t++) {
			...
			io[t].solveConstraints.dispatcher = dispatcher;
	...
	int actualNumManifolds= 0;
	{
		...
		//		float compFric = obA->getFriction()*obB->getFriction();//@todo
			...
			//		continue;
				...
				int contactId = m->m_index1a;
				btAssert(contactId>=0);
				btAssert(contactId<dispatcher->getNumManifolds());
	...
	//copy results back to bodies
	{
		...
		for (int i=0;i<numRigidBodies;i++)
		{
			...
			if (rb && (rb->getInvMass()>0.f))
			{
                ...
                // Record information about the amount of force applied to enforce each constraint.
                int numPoolConstraints = m_tmpSolverNonContactConstraintPool.size();
                for (int i = 0; i < numPoolConstraints; ++i)
                {
                    const btSolverConstraint &solverConstraint = m_tmpSolverNonContactConstraintPool[i];
                    btTypedConstraint *constraint = static_cast<btTypedConstraint *>(solverConstraint.m_originalContactPoint);
                    constraint->internalSetAppliedImpulse(solverConstraint.m_appliedImpulse);
                }
	}
