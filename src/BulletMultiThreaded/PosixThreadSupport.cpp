...
static int btSemWait(sem_t *sem)
{
    int rcode;
...
static void *threadFunction(void *argument) 
{
	...
	while (1)
	{
            checkPThreadFunction(btSemWait(status->startSemaphore));
...
void PosixThreadSupport::waitForResponse(unsigned int *puiArgument0, unsigned int *puiArgument1)
{
	...
	checkPThreadFunction(btSemWait(m_mainSemaphore));
...
void PosixThreadSupport::startThreads(ThreadConstructionInfo& threadConstructionInfo)
{
	...
	//checkPThreadFunction(btSemWait(mainSemaphore));
	...
	for (int i=0;i < threadConstructionInfo.m_numThreads;i++)
	{
		...
		spuStatus.startSemaphore = createSem("threadLocal");                
                spuStatus.mainSemaphore = m_mainSemaphore;
