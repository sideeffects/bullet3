...
void Win32ThreadSupport::startThreads(const Win32ThreadConstructionInfo& threadConstructionInfo)
{
	...
	for (int i=0;i<threadConstructionInfo.m_numThreads;i++)
	{
		...
		spuStatus.m_userPtr=0;
                ...
                // Don't create named events, since we might run into issues if
                // multiple instances of Win32ThreadSupport try to create
                // events with the same name.
		spuStatus.m_eventStartHandle = CreateEventA (0,false,false,0);
		spuStatus.m_eventCompletetHandle = CreateEventA (0,false,false,0);
